...
def prediction(image):
  ...
  image_inverted_bw_scaled = np.clip(image_bw_resized - min_pixel, 0, 255)
  max_pixel = np.max(image_bw_resized)
  image_inverted_bw_scaled = np.asarray(image_inverted_bw_scaled)/max_pixel

  testSample = np.array(image_inverted_bw_scaled).reshape(1, 660)
